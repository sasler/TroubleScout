...
#pragma warning restore CS0618
    ...
    [Fact]
    public void BuildSessionConfig_WhenMappedMcpRoleMissing_ShouldSurfaceCapabilityWarning()
    {
        ExecuteWithTemporarySettingsPath(_ =>
        {
            AppSettingsStore.Save(new AppSettings
            ...
            InvokeBuildSessionConfig(session, "gpt-4.1");

            session.GetStatusFields().Should().Contain(field =>
                field.Label == "Capability warnings"
                && field.Value.Contains("Monitoring MCP 'missing-zabbix' is not available in the current MCP configuration.", StringComparison.Ordinal));
        });
    }
    ...
    public async Task CreateSystemMessage_WithJeaContext_ShouldIncludePrimaryJeaBlock()
    {
        ...
        content.Should().Contain("delegate `run_delegated_powershell` with sessionName: \"server1\"");
        content.Should().Contain("Delegate run_delegated_powershell with sessionName: \"server1\" to target this JEA session.");
