...
internal static class ReportHtmlBuilder
{
        ...
        bool contentAlreadyRedacted = false)
    {
        ...
        sb.AppendLine("    body { font-family: 'Segoe UI', Inter, system-ui, -apple-system, sans-serif; margin: 0; background: #0b1020; color: #e2e8f0; line-height: 1.5; -webkit-font-smoothing: antialiased; }");
        sb.AppendLine("    .wrap { max-width: 1120px; margin: 0 auto; padding: 18px 18px 40px; }");

        // Hero header
        sb.AppendLine("    .hero { background: linear-gradient(135deg, #172033 0%, #101827 100%); border: 1px solid #2b3a55; border-radius: 12px; padding: 24px 28px 22px; margin-bottom: 18px; position: relative; overflow: hidden; }");
        sb.AppendLine("    .hero-top { display: flex; align-items: center; gap: 14px; margin-bottom: 16px; }");
        ...
        sb.AppendLine("    .hero-stats { display: grid; grid-template-columns: repeat(auto-fit, minmax(140px, 1fr)); gap: 10px; margin-top: 16px; }");
        sb.AppendLine("    .hero-stat { background: rgba(255,255,255,0.035); border: 1px solid #2b3a55; border-radius: 8px; padding: 10px 14px; text-align: center; }");
        ...
        // Summary cards
        sb.AppendLine("    .summary-row { display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 10px; margin-bottom: 22px; }");
        sb.AppendLine("    .summary-card { border-radius: 8px; padding: 12px 14px; border: 1px solid; }");
        sb.AppendLine("    .summary-card .sc-val { font-size: 1.5rem; font-weight: 700; }");
        ...
        // Prompt card
        sb.AppendLine("    .prompt-card { background: #101827; border: 1px solid #243146; border-radius: 10px; overflow: hidden; transition: border-color 0.2s ease; }");
        sb.AppendLine("    .prompt-card:hover { border-color: #334155; }");
        sb.AppendLine("    .prompt-header { padding: 14px 16px; cursor: pointer; display: flex; align-items: flex-start; gap: 12px; }");
        sb.AppendLine("    .prompt-header:hover { background: rgba(255,255,255,0.02); }");
        sb.AppendLine("    .prompt-badge { flex-shrink: 0; width: 32px; height: 32px; border-radius: 8px; background: linear-gradient(135deg, #2563eb, #4f46e5); display: flex; align-items: center; justify-content: center; font-weight: 700; font-size: 0.86rem; color: #fff; }");
        ...
        sb.AppendLine("    .prompt-body { padding: 0 16px 16px; border-top: 1px solid #243146; }");

        // Action card
        sb.AppendLine("    .action-card { background: #0c1424; border: 1px solid #243146; border-radius: 8px; padding: 12px; margin-top: 10px; }");
        sb.AppendLine("    .action-header { display: flex; flex-wrap: wrap; align-items: center; gap: 8px; margin-bottom: 10px; }");
        ...
        sb.AppendLine("    .inner-section { margin-top: 8px; border: 1px solid #243146; border-radius: 7px; overflow: hidden; background: #091120; }");
        sb.AppendLine("    .inner-section > summary { list-style: none; padding: 8px 12px; font-size: 0.78rem; font-weight: 700; letter-spacing: 0.035em; color: #93c5fd; text-transform: uppercase; cursor: pointer; display: flex; align-items: center; gap: 8px; }");
        ...
        sb.AppendLine("    .code-block { margin: 0; white-space: pre; word-break: normal; font-family: 'Cascadia Mono', Consolas, 'Courier New', monospace; font-size: 0.84rem; line-height: 1.35; border: 1px solid #243146; border-radius: 7px; padding: 10px 12px 10px 0; background: #070d19; counter-reset: line; overflow: auto; max-height: 520px; }");
        sb.AppendLine("    .code-line { display: block; padding-left: 52px; position: relative; min-height: 1.35em; }");
        ...
        sb.AppendLine("    .output-block { margin: 0; white-space: pre; word-break: normal; font-family: 'Cascadia Mono', Consolas, 'Courier New', monospace; font-size: 0.83rem; line-height: 1.35; border: 1px solid #243146; border-radius: 7px; padding: 10px 12px; background: #070d19; max-height: 360px; overflow: auto; }");
        sb.AppendLine("    .json-block { color: #dbeafe; }");
        ...
        sb.AppendLine("    .reply-bubble { margin-top: 12px; background: #111a2b; border: 1px solid #27364d; border-radius: 10px; padding: 14px 16px; position: relative; }");
        sb.AppendLine("    .reply-header { display: flex; align-items: center; gap: 8px; margin-bottom: 10px; }");
        sb.AppendLine("    .reply-avatar { width: 26px; height: 26px; border-radius: 7px; background: linear-gradient(135deg, #2563eb, #7c3aed); display: flex; align-items: center; justify-content: center; flex-shrink: 0; }");
        sb.AppendLine("    .reply-label { font-size: 0.82rem; font-weight: 700; color: #bfdbfe; }");
        ...
        // Session header card
        sb.AppendLine("    .session-header { background: #101827; border: 1px solid #243146; border-radius: 10px; padding: 14px 16px; margin-bottom: 16px; }");
        ...
        sb.AppendLine("    .session-grid { display: flex; flex-wrap: wrap; gap: 8px; font-size: 0.84rem; }");
        sb.AppendLine("    .session-grid > div { display: inline-flex; align-items: baseline; gap: 6px; max-width: 100%; padding: 5px 9px; border: 1px solid #2b3a55; border-radius: 999px; background: rgba(255,255,255,0.035); }");
        ...
        if (summary != null)
        {
            ...
            if (summary.AgentModels is { Count: > 0 })
            {
                ...
                sb.AppendLine($"        <div><dt>Subagent model</dt><dd>{HtmlEncode(FormatModelNameForReport(subagentModel))}</dd></div>");
            }
            ...
            else
            {
                foreach (var action in prompt.Actions)
                {
                    ...
                    if (isMcp || isDelegationAuthorization || isSubagentTool || isTool)
                    {
                        ...
                        {
                            AppendArgumentsSection(sb, action.Arguments!);
                        }
        ...
        sb.AppendLine("      if (pre.classList.contains('code-block')) {");
        sb.AppendLine("        var lines = Array.prototype.slice.call(pre.querySelectorAll('.code-line')).map(function(line) { return line.textContent || ''; });");
    ...
    {
        command = TrimOuterBlankLines(command);
        if (string.IsNullOrEmpty(command))
        {
        ...
        {
            sb.Append("<span class=\"code-line\">").Append(line).Append("</span>");
        }
        return sb.ToString();
    }

    private static void AppendArgumentsSection(StringBuilder sb, string arguments)
    {
        var isJson = TryFormatJson(arguments, out var formattedJson);
        var displayText = isJson
            ? formattedJson!
            : arguments.Trim();
        var blockClass = formattedJson == null
            ? "output-block"
            : "output-block json-block language-json";

        sb.AppendLine("              <details class=\"inner-section\" data-md-section=\"Arguments\">");
        sb.AppendLine("                <summary>Arguments</summary>");
        sb.AppendLine("                <div class=\"inner-content\">");
        sb.AppendLine($"                  <div class=\"code-wrap\"><button class=\"copy-btn\" onclick=\"copyCode(this)\">Copy</button><pre class=\"{blockClass}\">{HtmlEncode(displayText)}</pre></div>");

        if (TryExtractPowerShellArgument(arguments, out var powerShell))
        {
            sb.AppendLine("                  <details class=\"inner-section\" data-md-section=\"Extracted PowerShell\" open>");
            sb.AppendLine("                    <summary>Extracted PowerShell</summary>");
            sb.AppendLine("                    <div class=\"inner-content\">");
            sb.AppendLine($"                      <div class=\"code-wrap\"><button class=\"copy-btn\" onclick=\"copyCode(this)\">Copy</button><pre class=\"code-block\">{RenderCommandHtmlWithLineNumbers(powerShell!)}</pre></div>");
            sb.AppendLine("                    </div>");
            sb.AppendLine("                  </details>");
        }
    ...
    private static bool TryFormatJson(string raw, out string? formattedJson)
    {
        ...
        try
        {
            ...
            formattedJson = JsonSerializer.Serialize(doc.RootElement, IndentedJsonOptions);
            return true;
        }
        catch (JsonException)
        {
    ...
    private static bool TryExtractPowerShellArgument(string raw, out string? powerShell)
    {
        powerShell = null;
        if (string.IsNullOrWhiteSpace(raw))
        {
        ...
        try
        {
            ...
            foreach (var propertyName in new[] { "input", "command", "script" })
            {
    ...
    private static bool LooksLikePowerShell(string? value)
    {
        ...
        return text.Contains('$', StringComparison.Ordinal)
            ...
            || Regex.IsMatch(text, @"\b[A-Za-z]+-[A-Za-z][A-Za-z0-9]*\b");
    }
    ...
    private static string GetReplyLabel(string reply)
    {
        if (IsApprovalRequestReply(reply))
        {
    ...
    private static bool IsApprovalRequestReply(string reply)
    {
        ...
        return reply.Contains("Awaiting your approval", StringComparison.OrdinalIgnoreCase)
            || reply.Contains("Please approve", StringComparison.OrdinalIgnoreCase)
            || reply.Contains("pending command", StringComparison.OrdinalIgnoreCase);
    }

    private static string FormatModelNameForReport(string model)
    {
        ...
        static string TitlePart(string part) => CultureInfo.InvariantCulture.TextInfo.ToTitleCase(part);

        if (parts[0].Equals("gpt", StringComparison.OrdinalIgnoreCase) && parts.Length > 1)
        {
            var suffix = parts.Length == 2
                ? string.Empty
                : " " + string.Join(' ', parts.Skip(2).Select(TitlePart));
            return $"GPT-{parts[1]}{suffix}";
        }
